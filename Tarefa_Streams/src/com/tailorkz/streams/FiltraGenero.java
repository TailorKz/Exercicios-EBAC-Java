...
public class FiltraGenero {
    public static void main(String[] args) {
        ...
        String[] pessoas = valores.split(",");

        List<String> feminino = Arrays.stream(pessoas).map(String::trim).filter(pessoa -> {
            String[] partes = pessoa.split("-");
            return partes[1].equalsIgnoreCase("F");
        }).map(pessoa -> pessoa.split("-")[0]).collect(Collectors.toList());
